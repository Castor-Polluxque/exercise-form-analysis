...
ACTIVITY_CONFIGS: Dict[str, Dict[str, Any]] = {
    "pushups": {
        "candidate_signals": ["z", "y", "mag", "x"],
        "lowpass_hz": 4.0,
        "prominence_factor": 0.30,
        "min_dur_factor": 0.55,
        "prefer_devices": ["samsung_phone", "garmin_watch"],
    },
    "situps": {
        "candidate_signals": ["x", "z", "mag", "y"],
        "lowpass_hz": 3.5,
        "prominence_factor": 0.28,
        "min_dur_factor": 0.60,
    ...
    "squats": {
        ...
        "lowpass_hz": 3.0,
        "prominence_factor": 0.38,
        "min_dur_factor": 0.58,
        "prefer_devices": ["garmin_watch", "bose_headphones", "samsung_phone"],
    },
    "bench": {
        "candidate_signals": ["y", "z", "mag", "x"],
        "lowpass_hz": 3.5,
        "prominence_factor": 0.32,
        "min_dur_factor": 0.56,
        "prefer_devices": ["garmin_watch", "samsung_phone"],
    },
    "lunges": {
        "candidate_signals": ["mag", "y", "z", "x"],
        "lowpass_hz": 3.0,
...
def _detect_events_on_signal(
    ...
    *,
    ...
    if min_separation_s is None:
        period_s = plot_multi.estimate_rep_period_acf(centred, fs)
    ...
    *,
    ...
    if best is None:
        ...
        best = {
            ...
            "regularity": _segment_regularity(plot_multi.segments_to_time_windows(segments, times)),
...
def _choose_primary_device(
    ...
    def _score(i: int) -> Tuple[float, float, float, float]:
        ...
        return (regularity_score, duration_score, float(seg_count), fs)
...
def analyze_with_plot_multi(
    ...
    primary_idx = _choose_primary_device(devices, template_reps=template_reps, expected_reps=expected_reps)
...
def main() -> None:
    ...
    for session_dir in _iter_session_dirs(root):
        ...
        try:
            analyzer = _preferred_analyzer(session_dir)
            analyses: List[Dict[str, Any]] = []
            chosen_analysis_result: Optional[Dict[str, Any]] = None

            try:
                analyses.append(analyze_with_finalrep(session_dir))
            except Exception:
                pass
            try:
                analyses.append(
            ...
            if not analyses:
                raise ValueError(f"No analyzer succeeded for {session_dir}")

            analysis = _choose_best_analysis(analyses, expected_reps=expected_reps, preferred_analyzer=analyzer)
