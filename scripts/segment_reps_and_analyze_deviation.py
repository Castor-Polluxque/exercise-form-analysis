...
typically the wrist) using valley-to-valley boundaries. The first N reps
...
def detect_peaks_valleys(times, xs, ys, zs,
    ...
    Steps:
      1. Compute vector magnitude: mag = sqrt(x² + y² + z²)
      2. Low-pass filter at `lowpass_hz` Hz to remove high-freq noise
      3. Run scipy find_peaks on the filtered magnitude with:
           - minimum separation of `min_separation_s` seconds
           - minimum prominence of `prominence_factor` × signal IQR
      4. Run find_peaks on the inverted signal for valleys
    ...
    dist_samples = max(1, int(min_separation_s * fs))
...
def main():
    ...
    parser.add_argument('--min-separation', type=float, default=1.0,
                        help='Minimum seconds between peaks/valleys '
                             '(default: 1.0 s)')
    ...
    for d in all_data:
        ...
        print(d['label'])
        peak_times, valley_times, valley_idx, mag_f = detect_peaks_valleys(
            times, xs, ys, zs,
